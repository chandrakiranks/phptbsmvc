<?php
	...
	class db {
		...
		// Chandra : changed to implement db class in PHP 5

		private static $instance;

		var $dbuser     = 'root';
		var $dbpassword = 'nmklop90';
		var $dbname     = 'sample';
...
//			if ( !@mysql_select_db($db,$this->dbh))
		...
		function escape($str)
		{
			if ( is_array($str) )
			{
				...
				{
					if ( is_string($val) )
//						$str[$key] = mysql_escape_string ( stripslashes(trim($val)) );
						$str[$key] = mysqli_real_escape_string($this->dbh, stripslashes(trim($val)));
				}
			}

			if ( is_string($str) ){
                $str = mysqli_real_escape_string($this->dbh, stripslashes(trim($str)));
            }
		...
		// =======================================================
		// Returns an array with enum values
